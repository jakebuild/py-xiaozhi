#!/usr/bin/env python3
"""
四阶段Initialization流程测试脚本 展示设备身份准备、Configuration管理、OTAConfigurationGet三个阶段的协调工作 激活流程由用户自己实现.
"""
...
class SystemInitializer:
    ...
    async def run_initialization(self) -> Dict:
        """运行完整的Initialization流程.

        Returns:
            Dict: Initialization结果，包含激活状态和是否需要激活界面
        """
        logger.info("开始系统Initialization流程")

        try:
            # 第一阶段：设备身份准备
            await self.stage_1_device_fingerprint()

            # 第二阶段：Configuration管理Initialization
            await self.stage_2_config_management()

            # 第三阶段：OTAGetConfiguration
            await self.stage_3_ota_config()

            # Get激活版本Configuration
            ...
            if activation_version == "v1":
                # v1协议：Complete前三个阶段后直接返回Success
                logger.info("v1协议：前三个阶段Complete，无需激活流程")
                return {
                    "success": True,
                    ...
                    "status_message": "v1协议InitializationComplete",
            ...
            logger.error(f"系统InitializationFailure: {e}")
    ...
    async def stage_1_device_fingerprint(self):
        ...
        logger.info(f"本地激活Status: {'已激活' if is_activated else '未激活'}")
    ...
    async def stage_3_ota_config(self):
        ...
        # Get并UpdateConfiguration
        try:
            config_result = await self.ota.fetch_and_update_config()

            logger.info("OTAConfigurationGet结果:")
            mqtt_status = "已Get" if config_result["mqtt_config"] else "未Get"
            logger.info(f"- MQTTConfiguration: {mqtt_status}")

            ws_status = "已Get" if config_result["websocket_config"] else "未Get"
            logger.info(f"- WebSocketConfiguration: {ws_status}")

            # 显示Get到的Configuration信息摘要
            response_data = config_result["response_data"]
            # 详细Configuration信息仅在调试模式下显示
            logger.debug(
            ...
            logger.error(f"OTAConfigurationGetFailure: {e}")
            raise
    ...
    def analyze_activation_status(self) -> Dict:
        ...
        # 情况3: 本地未激活，但服务器无激活数据 - 状态不一致，自动修复
            ...
            result["local_activated"] = True  # Update结果中的状态
    ...
    async def _run_gui_activation(self) -> Dict:
        ...
        try:
            ...
            logger.error(f"GUI激活流程Exception: {e}", exc_info=True)
    ...
    async def _run_cli_activation(self) -> Dict:
        ...
        try:
            ...
            logger.error(f"CLI激活流程Exception: {e}", exc_info=True)
            return {"is_activated": False, "error": str(e)}
