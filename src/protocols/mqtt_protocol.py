...
class MqttProtocol(Protocol):
    def __init__(self, loop):
        ...
        self._connection_timeout = 120  # Connection timeout检测（秒）
    ...
    async def connect(self):
        ...
        # 首先尝试GetMQTTConfiguration
        try:
            # 尝试从OTA服务器GetMQTTConfiguration
            ...
            logger.info(f"已从OTA服务器GetMQTTConfiguration: {self.endpoint}")
        except Exception as e:
            logger.warning(f"从OTA服务器GetMQTTConfigurationFailure: {e}")
        ...
        if (
            ...
            logger.error("MQTTConfiguration不完整")
            if self._on_network_error:
                await self._on_network_error("MQTTConfiguration不完整")
            return False
        ...
        # 如果已有MQTT客户端，先DisconnectConnect
        if self.mqtt_client:
            try:
                self.mqtt_client.loop_stop()
                self.mqtt_client.disconnect()
            except Exception as e:
                logger.warning(f"DisconnectMQTT客户端Connect时出错: {e}")

        # Parseendpoint，提取主机和端口
        try:
            host, port = self._parse_endpoint(self.endpoint)
            use_tls = port == 8883  # 只有使用8883端口时才使用TLS

            logger.info(
                f"Parseendpoint: {self.endpoint} -> 主机: {host}, 端口: {port}, 使用TLS: {use_tls}"
            )
        except ValueError as e:
            logger.error(f"ParseendpointFailure: {e}")
            if self._on_network_error:
                await self._on_network_error(f"ParseendpointFailure: {e}")
            return False
        ...
        # 根据端口决定是否ConfigurationTLS加密Connect
        if use_tls:
            try:
                ...
                logger.info("已ConfigurationTLS加密Connect")
            except Exception as e:
                logger.error(f"TLSConfigurationFailure，无法安全Connect到MQTT服务器: {e}")
                if self._on_network_error:
                    await self._on_network_error(f"TLSConfigurationFailure: {str(e)}")
                return False
        else:
            logger.info("使用非TLSConnect")
        ...
        def on_connect_callback(client, userdata, flags, rc, properties=None):
            if rc == 0:
                logger.info("Connected到MQTT服务器")
                self._last_activity_time = time.time()
                self.loop.call_soon_threadsafe(lambda: connect_future.set_result(True))
            else:
                logger.error(f"ConnectMQTT服务器Failure，返回码: {rc}")
                self.loop.call_soon_threadsafe(
                    lambda: connect_future.set_exception(
                        Exception(f"ConnectMQTT服务器Failure，返回码: {rc}")
                    )
                )

        def on_message_callback(client, userdata, msg):
            try:
                self._last_activity_time = time.time()  # Update活动时间
        ...
        def on_disconnect_callback(client, userdata, rc):
            """MQTTDisconnectConnect回调.

            Args:
                client: MQTT客户端实例
                userdata: 用户数据
                rc: 返回码 (0=正常Disconnect, >0=ExceptionDisconnect)
            """
            try:
                if rc == 0:
                    logger.info("MQTTConnect正常Disconnect")
                else:
                    logger.warning(f"MQTTConnectExceptionDisconnect，返回码: {rc}")
                ...
                if self._on_connection_state_changed and was_connected:
                    reason = "正常Disconnect" if rc == 0 else f"ExceptionDisconnect(rc={rc})"
                    self.loop.call_soon_threadsafe(
                ...
                # 只有在ExceptionDisconnect且启用自动Reconnect时才尝试Reconnect
                if (
                    rc != 0
                    ...
                    self.loop.call_soon_threadsafe(
                        lambda: asyncio.create_task(
                            self._attempt_reconnect(f"MQTTDisconnect(rc={rc})")
                        )
                    )
                else:
                    # 通知音频通道Close
                    ...
                    if rc != 0 and self._on_network_error:
                        error_msg = f"MQTTConnectDisconnect: {rc}"
                        if (
                            self._auto_reconnect_enabled
                            and self._reconnect_attempts >= self._max_reconnect_attempts
                        ):
                            error_msg += " (ReconnectFailure)"
                        self.loop.call_soon_threadsafe(
                            lambda: self._on_network_error(error_msg)
                        )

            except Exception as e:
                logger.error(f"处理MQTTDisconnectConnection failed: {e}")
        ...
        def on_subscribe_callback(client, userdata, mid, granted_qos):
            ...
            MQTT订阅回调.
            """
            logger.info(f"订阅Success，主题: {self.subscribe_topic}")
            self._last_activity_time = time.time()  # Update活动时间
        ...
        try:
            # ConnectMQTT服务器，Configuration保活间隔
            logger.info(f"ConnectingMQTT服务器: {host}:{port}")
            ...
            if not await self.send_text(json.dumps(hello_message)):
                logger.error("发送hello消息Failure")
                return False

            try:
                await asyncio.wait_for(self.server_hello_event.wait(), timeout=10.0)
            except asyncio.TimeoutError:
                logger.error("等待服务器hello消息Timeout")
                if self._on_network_error:
                    await self._on_network_error("等待响应Timeout")
                return False
            ...
            try:
                ...
                logger.error(f"创建UDP套接字Failure: {e}")
                if self._on_network_error:
                    await self._on_network_error(f"创建UDPConnection failed: {e}")
                return False

        except Exception as e:
            logger.error(f"ConnectMQTT服务器Failure: {e}")
            if self._on_network_error:
                await self._on_network_error(f"ConnectMQTT服务器Failure: {e}")
            return False
    ...
    def _handle_mqtt_message(self, payload):
        ...
        try:
            ...
            if msg_type == "goodbye":
                ...
                print("服务链接返回InitializationConfiguration", data)
                # 处理服务器hello响应
                ...
                udp = data.get("udp")
                if not udp:
                    logger.error("UDPConfiguration缺失")
                    return
    ...
    def _udp_receive_thread(self):
        ...
        """
        logger.info(
            f"UDP接收线程Started，监听来自 {self.udp_server}:{self.udp_port} 的数据"
        )
        ...
        while self.udp_running:
            try:
                ...
                logger.error(f"UDP接收线程Error: {e}")
                if not self.udp_running:
                    break
                time.sleep(0.1)  # 避免在Error情况下过度消耗CPU

        logger.info("UDP接收线程已Stop")
    ...
    async def send_text(self, message):
        ...
        if not self.mqtt_client:
            logger.error("MQTT客户端未Initialization")
            return False
        ...
        try:
            ...
            logger.error(f"发送MQTT消息Failure: {e}")
            if self._on_network_error:
                await self._on_network_error(f"发送MQTT消息Failure: {e}")
            return False
    ...
    async def send_audio(self, audio_data):
        ...
        if not self.udp_socket or not self.udp_server or not self.udp_port:
            logger.error("UDP通道未Initialization")
            return False
        ...
        try:
            ...
            if self._on_network_error:
                asyncio.create_task(self._on_network_error(f"发送音频数据Failure: {e}"))
            return False
    ...
    def is_audio_channel_opened(self) -> bool:
        ...
        更准确地检查Connect状态，包括MQTT和UDP的实际状态
        """
        ...
        try:
            ...
            if self.udp_socket:
                try:
                    ...
                    logger.error(f"CloseUDP套接字Failure: {e}")
                self.udp_socket = None
            ...
            if self.mqtt_client:
                try:
                    self.mqtt_client.loop_stop()
                    self.mqtt_client.disconnect()
                    self.mqtt_client.loop_forever()  # 确保DisconnectConnect完全Complete
                except Exception as e:
                    logger.error(f"DisconnectMQTTConnection failed: {e}")
                self.mqtt_client = None
        ...
        try:
            while self.connected and not self._is_closing:
                ...
                if self.mqtt_client and not self.mqtt_client.is_connected():
                    logger.warning("检测到MQTTConnect已Disconnect")
                    await self._handle_connection_loss("MQTTConnect检测Failure")
                    break
                ...
                if self._last_activity_time:
                    ...
                    if time_since_activity > self._connection_timeout:
                        logger.warning(
                            f"Connection timeout，最后活动时间: {time_since_activity:.1f}秒前"
                        )
                        await self._handle_connection_loss("Connection timeout")
                        break

        except asyncio.CancelledError:
            logger.debug("MQTTConnect监控任务被取消")
        except Exception as e:
            logger.error(f"MQTTConnect监控Exception: {e}")

    async def _handle_connection_loss(self, reason: str):
        """
        处理Connect丢失.
        """
        logger.warning(f"MQTTConnect丢失: {reason}")
        ...
        if (
            ...
            if self._on_network_error:
                if (
                    ...
                    await self._on_network_error(f"MQTTConnect丢失且ReconnectFailure: {reason}")
                else:
                    await self._on_network_error(f"MQTTConnect丢失: {reason}")
    ...
    async def _attempt_reconnect(self, original_reason: str):
        ...
        logger.info(
            f"尝试MQTT自动Reconnect ({self._reconnect_attempts}/{self._max_reconnect_attempts})"
        )

        # 等待一段时间后Reconnect（指数退避）
        await asyncio.sleep(min(self._reconnect_attempts * 2, 30))

        try:
            success = await self.connect()
            if success:
                logger.info("MQTT自动ReconnectSuccess")
                # 通知Connect状态变化
                if self._on_connection_state_changed:
                    self._on_connection_state_changed(True, "ReconnectSuccess")
            else:
                logger.warning(
                    f"MQTT自动ReconnectFailure ({self._reconnect_attempts}/{self._max_reconnect_attempts})"
                )
                # 如果还能重试，不立即报错
                if self._reconnect_attempts >= self._max_reconnect_attempts:
                    if self._on_network_error:
                        await self._on_network_error(
                            f"MQTTReconnectFailure，已达到最大Reconnect次数: {original_reason}"
                        )
        except Exception as e:
            logger.error(f"MQTTReconnect过程中出错: {e}")
            if self._reconnect_attempts >= self._max_reconnect_attempts:
                if self._on_network_error:
                    await self._on_network_error(f"MQTTReconnectException: {str(e)}")
    ...
    def enable_auto_reconnect(self, enabled: bool = True, max_attempts: int = 5):
        ...
        if enabled:
            ...
            logger.info(f"启用MQTT自动Reconnect，最大尝试次数: {max_attempts}")
        else:
            self._max_reconnect_attempts = 0
            logger.info("禁用MQTT自动Reconnect")
