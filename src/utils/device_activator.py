...
class DeviceActivator:
    ...
    def _ensure_device_identity(self):
        ...
        self.logger.info(
            f"设备身份信息: 序列号: {serial_number}, 激活Status: {'已激活' if is_activated else '未激活'}"
        )
        ...
        try:
            ...
            if not self.has_serial_number():
                ...
                if serial_number and hmac_key:
                    ...
                    self.logger.error("创建序列号或HMAC密钥Failure")
                    return False
            ...
            try:
                ...
                self.logger.error(f"播放验证码语音Failure: {e}")
    ...
    async def activate(self, challenge: str, code: str = None) -> bool:
        ...
        try:
            ...
            if not serial_number:
                self.logger.error("设备没有序列号，无法CompleteHMAC验证步骤")
                return False

            # 计算HMAC签名
            hmac_signature = self.generate_hmac(challenge)
            if not hmac_signature:
                self.logger.error("无法生成HMAC签名，激活Failure")
                return False
            ...
            if not ota_url:
                self.logger.error("未找到OTA URLConfiguration")
                return False
                    ...
                    try:
                        ...
                        if attempt > 0 and code:
                            try:
                                ...
                                self.logger.error(f"重试播放验证码Failure: {e}")
                            ...
                            if response.status == 200:
                                ...
                                try:
                                    ...
                                    error_msg = error_data.get(
                                        "error", f"未知Error (状态码: {response.status})"
                                    )
                                except json.JSONDecodeError:
                                    error_msg = (
                                        f"服务器返回Error (状态码: {response.status})"
                                    )
                                ...
                                if "Device not found" in error_msg:
                                    ...
                                    if error_count >= 5 and error_count % 5 == 0:
                                        self.logger.warning(
                                            "\n提示: 如果Error持续出现，可能需要在网站上Refresh页面Get新验证码\n"
                                        )
                        ...
                        self.logger.warning(f"网络请求Failure: {e}，重试中...")
                        await asyncio.sleep(retry_interval)

                    except asyncio.TimeoutError as e:
                        self.logger.warning(f"请求Timeout: {e}，重试中...")
                        await asyncio.sleep(retry_interval)
                        ...
                        error_detail = (
                            str(e) if str(e) else f"{type(e).__name__}: 未知Error"
                        )
                        self.logger.warning(
                            f"激活过程中发生Error: {error_detail}，重试中..."
                        )
                        # 调试模式下打印完整的Exception信息
                        self.logger.debug(f"完整Exception信息: {traceback.format_exc()}")
                        await asyncio.sleep(retry_interval)

            # 达到最大重试次数
            self.logger.error(
                f"激活Failure，达到最大重试次数 ({max_retries})，最后Error: {last_error}"
            )
            return False
