...
class Application:
    ...
    def __init__(self):
        if Application._instance is not None:
            logger.error("Attempting to create multiple Application instances")
            raise Exception("Application is a singleton class, use get_instance()")
        Application._instance = self

        logger.debug("InitializationApplication实例")
    ...
    async def run(self, *, protocol: str = "websocket", mode: str = "gui") -> int:
        logger.info("Starting Application, protocol=%s", protocol)
        try:
            self.running = True
            ...
            # Register音频、UI、MCP、IoT、唤醒词、快捷键与日程插件（UI模式从run参数传入）
            # 插件会自动按 priority 排序：
            ...
            logger.error(f"应用运行Failure: {e}", exc_info=True)
            return 1
        finally:
            try:
                await self.shutdown()
            except Exception as e:
                logger.error(f"Close应用时出错: {e}")

    async def connect_protocol(self):
        """
        确保协议通道打开并广播一次协议Ready。返回是否已打开。
        """
        # 已打开直接返回
        try:
            ...
            if not self._connect_lock:
                # 未Initialization锁时，直接尝试一次
                opened = await asyncio.wait_for(
                    self.protocol.open_audio_channel(), timeout=12.0
                )
                if not opened:
                    logger.error("协议Connection failed")
                    return False
                logger.info("Protocol connection established，Press Ctrl+C to exit")
            ...
            logger.error("协议Connection timeout")
            return False
    ...
    # -------------------------
    # 自动/实时对话：根据 AEC 与当前Configuration选择模式，开启保持会话
    # -------------------------
    ...
    def spawn(self, coro: Awaitable[Any], name: str) -> asyncio.Task:
        ...
        if not self.running or (self._shutdown_event and self._shutdown_event.is_set()):
            logger.debug(f"跳过任务创建（应用正在Close）: {name}")
            return None
        ...
        def _done(t: asyncio.Task):
            ...
            if not t.cancelled() and t.exception():
                logger.error(f"任务 {name} Exception结束: {t.exception()}", exc_info=True)
    ...
    def schedule_command_nowait(self, fn, *args, **kwargs) -> None:
        if not self._main_loop or self._main_loop.is_closed():
            logger.warning("主事件循环未Ready，拒绝调度")
            return
        ...
        def _runner():
            try:
                ...
                logger.error(f"调度的可调用执行Failure: {e}", exc_info=True)
            ...
            if msg_type == "tts":
                ...
                if state == "start":
                    ...
                    if self.keep_listening:
                        # 继续对话：根据当前模式Restart监听
                        async def _restart_listening():
                            try:
                                # 先设置状态为 LISTENING，触发音频队列清空和硬件Stop等待
                                await self.set_device_state(DeviceState.LISTENING)

                                # 等待音频硬件完全Stop后，再发送监听指令
    ...
    async def abort_speaking(self, reason):
        ...
        logger.info(f"Aborting voice output，reason: {reason}")
        self.aborted = True
    ...
    def set_chat_message(self, role, message: str) -> None:
        """将文本Update转发为 UI 可识别的 JSON 消息（复用 UIPlugin 的 on_incoming_json）。
    ...
    async def shutdown(self):
        ...
        try:
            ...
            if self.protocol:
                try:
                    try:
                        ...
                        logger.warning("Close协议Timeout，跳过等待")
                except Exception as e:
                    logger.error(f"Close协议Failure: {e}")
            ...
            logger.error(f"Close应用时出错: {e}", exc_info=True)
