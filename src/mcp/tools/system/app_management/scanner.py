...
async def scan_installed_applications(args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.info(f"[AppScanner] 开始扫描已安装应用程序，强制Refresh: {force_refresh}")
        ...
        result = {
            ...
            "applications": apps,
            "message": f"Success扫描到 {len(apps)} 个已安装应用程序",
        }

        logger.info(f"[AppScanner] 扫描Complete，找到 {len(apps)} 个应用程序")
        return json.dumps(result, ensure_ascii=False, indent=2)

    except Exception as e:
        error_msg = f"扫描应用程序Failure: {str(e)}"
...
async def list_running_applications(args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.info(f"[AppScanner] 列出Complete，找到 {len(apps)} 个正在运行的应用程序")
        return json.dumps(result, ensure_ascii=False, indent=2)

    except Exception as e:
        error_msg = f"列出运行应用程序Failure: {str(e)}"
        logger.error(f"[AppScanner] {error_msg}", exc_info=True)
        return json.dumps(
            {
                "success": False,
